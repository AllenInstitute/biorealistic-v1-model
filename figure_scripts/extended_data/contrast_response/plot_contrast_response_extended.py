"""Extended Data Figure: Contrast response line plots for all cell types.

Aggregates across all core_nll_* networks (bio_trained condition by default).
...
Output: figures/extended_data/contrast_response/
"""
...
def compute_dsi(rates: np.ndarray, angles: np.ndarray) -> float:
    """Direction selectivity index: |sum(r * exp(i*theta))| / sum(r)."""
    if rates.sum() == 0:
        return np.nan
    rad = np.deg2rad(angles)
...
def compute_lifetime_sparsity(rates: np.ndarray) -> float:
    """Stimulus selectivity across orientations."""
    n = len(rates)
...
def process_one_network(basedir: str, network_option: str) -> pd.DataFrame | None:
    ...
    for cell_type, df_type in nodes_core.groupby("cell_type"):
        ...
        for c_idx, contrast in enumerate(contrast_vals):
            ...
            mean_by_angle = contrast_rates.mean(axis=1)

            rate_values = mean_by_angle.mean(axis=0)          # per-cell mean rate
...
def make_figure(
    ...
    for ax_idx, (layer_name, cell_types) in enumerate(LAYER_GROUPS.items()):
        ...
        # Legend inside each panel, small font
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extended Data: contrast response line plots for all cell types."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--conditions", nargs="*", default=["bio_trained"],
        help="Training conditions to plot (default: bio_trained).",
    )
    ...
    save_dir = Path(ROOT) / "figures" / "extended_data" / "contrast_response"
