...
from analysis_shared.pd import compute_pd_ei2x2_cache, plot_pd_ei2x2_from_cache
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate 2x2 E/I PD matrix with caching (simulation only)."
    )
    ap.add_argument(
        "--bases",
        nargs="*",
    ...
    ap.add_argument(
        "--out", default="figures/paper/pd_sim_ei2x2_cached.png", help="Output PNG path"
    )
    ap.add_argument("--bin-step", type=float, default=20.0, help="Bin step in degrees")
    ap.add_argument(
        "--no-simplify-inh",
        action="store_true",
    ...
    ap.add_argument("--cache-name", default="pd_sim_ei2x2.pkl", help="Cache file name")
    ap.add_argument(
        "--replot-only",
        action="store_true",
    ...
    plot_pd_ei2x2_from_cache(cache, args.out)
