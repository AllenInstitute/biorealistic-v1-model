...
from analysis_shared.pd import (
    ...
    plot_pd_full_matrix_from_cache,
)
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate full simulation-only PD matrix (hist+fit) across Exc + aggregated Inh, with caching."
    )
    ap.add_argument(
        "--bases",
        nargs="*",
    ...
    ap.add_argument(
        "--out", default="figures/paper/pd_sim_full_matrix.png", help="Output PNG path"
    )
    ...
    ap.add_argument(
        "--cache-name", default="pd_sim_full_matrix.pkl", help="Cache file name"
    )
    ap.add_argument(
        "--replot-only",
        action="store_true",
    ...
    plot_pd_full_matrix_from_cache(cache, args.out)
