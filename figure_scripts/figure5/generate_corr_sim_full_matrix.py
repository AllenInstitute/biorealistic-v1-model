...
from analysis_shared.corr import (
    ...
    plot_corr_full_matrix_from_cache,
)
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate full simulation-only Corr matrix (bar+OLS) across Exc + aggregated Inh, with caching."
    )
    ap.add_argument(
        "--bases",
        nargs="*",
    ...
    ap.add_argument(
        "--out",
        default="figures/paper/corr_sim_full_matrix_inhagg.png",
    ...
    ap.add_argument(
        "--cache-name", default="corr_sim_full_matrix.pkl", help="Cache file name"
    )
    ap.add_argument(
        "--replot-only",
        action="store_true",
    ...
    plot_corr_full_matrix_from_cache(cache, args.out)
