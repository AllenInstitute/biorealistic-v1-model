#!/usr/bin/env python3
"""Create node sets for high/low incoming *weight* cohorts (Extended Data for Figure 5).

Mirrors `analysis_shared/create_highlow_outgoing_weight_nodesets.py` but uses the
sum of absolute synaptic weights per *target* neuron (total incoming |syn_weight|).

Outputs are written to `<base_dir>/node_sets/` with naming:
- high_incoming_exc_core_nodes.json
- low_incoming_exc_core_nodes.json
- pv_high_incoming_core_nodes.json
- pv_low_incoming_core_nodes.json
  etc.
"""
...
def load_incoming_abs_weight(edge_file: Path) -> pd.Series:
    """Sum absolute synaptic weights per *target* neuron."""
    ...
    n_nodes = int(targets.max()) + 1 if targets.size else 0
    sums = safe_bincount_sum(targets, abs_w, minlength=n_nodes)
    return pd.Series(sums, name="incoming_weight_abs")


def _node_set_filename(base: str, weight: str, region: str) -> str:
    if base in {"pv", "sst", "vip"}:
        return f"{base}_{weight}_incoming_{region}_nodes.json"
    return f"{weight}_incoming_{base}_{region}_nodes.json"
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Create high/low incoming weight node sets for Extended Data (Fig. 5)."
    )
    ap.add_argument(
        "--base-dir",
        type=Path,
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Edge file to compute incoming weights from (default: <base_dir>/network/v1_v1_edges_bio_trained.h5).",
    )
    ap.add_argument(
        "--mode",
...
def main() -> None:
    ...
    print(f"Computing incoming |weight| from: {edge_file}")
    incoming = load_incoming_abs_weight(edge_file)
    ...
    if args.mode == "by_cell_type":
        ...
        high_exc, low_exc, exc_stats = _select_high_low_by_cell_type(exc_core, incoming)
        high_inh, low_inh, inh_stats = _select_high_low_by_cell_type(inh_core, incoming)
        results.extend({"base": "exc", "region": "core", **rec} for rec in exc_stats)
        results.extend({"base": "inh", "region": "core", **rec} for rec in inh_stats)

        files_to_write = {
            "high_incoming_exc_core_nodes.json": sorted(set(high_exc)),
            "low_incoming_exc_core_nodes.json": sorted(set(low_exc)),
            "high_incoming_inh_core_nodes.json": sorted(set(high_inh)),
            "low_incoming_inh_core_nodes.json": sorted(set(low_inh)),
        }
        ...
        if results:
            ...
            out_stats = node_set_dir / "incoming_weight_nodeset_stats_by_celltype.csv"
    ...
    for base, base_df in base_defs.items():
        ...
        for region, mask in (("core", base_df["core"]), ("periphery", ~base_df["core"])):
            ...
            high_ids, low_ids, stats = _select_high_low_by_layer(subset, incoming)
            for rec in stats:
            ...
            if base == "exc":
                dup_high = node_set_dir / f"high_incoming_exc_{region}.json"
                dup_low = node_set_dir / f"low_incoming_exc_{region}.json"
            elif base == "inh":
                dup_high = node_set_dir / f"high_incoming_inh_{region}.json"
                dup_low = node_set_dir / f"low_incoming_inh_{region}.json"
            else:
                dup_high = None
                dup_low = None
        ...
        if base in {"exc", "inh"}:
            ...
            if base == "exc":
                alias_high = node_set_dir / "high_incoming_exc_nodes.json"
                alias_low = node_set_dir / "low_incoming_exc_nodes.json"
            else:
                alias_high = node_set_dir / "high_incoming_inh_nodes.json"
                alias_low = node_set_dir / "low_incoming_inh_nodes.json"
    ...
    if results:
        stats_df = pd.DataFrame(results)
        out_stats = node_set_dir / "incoming_weight_nodeset_stats.csv"
        stats_df.to_csv(out_stats, index=False)
        print(f"\nSaved selection summary: {out_stats}")
